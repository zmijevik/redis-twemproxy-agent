...
function Agent(config){
  ...
  this.notify_command         = config.notify_command;
...
Agent.prototype.notify = function(title, description) {
  var self = this;
  process.env.RT_AGENT_SUBJECT = title;
  process.env.RT_AGENT_MESSAGE = desription;
  var child = exec(
    this.notify_command,
    function(error, stdout, stderr) {
      self.log("Notification sent of: " + title);
      self.log(stdout);
      if (error !== null) {
        self.log("Experienced some error sending a notification: " + error);
      }
      return callback();
    }
  );
...
Agent.prototype.start_sentinel = function(){
  ...
  this.client.on("error", function(msg) {
     ...
     if (msg.toString().indexOf("ECONNREFUSED") == -1) {
         error_msg = "Redis TwemProxy Agent encountered an error: ";
     } else {
         self.conn_retry_count = self.conn_retry_count + 1;
         if (self.conn_retry_count % 10 == 0) {
             error_msg = "WARNING: Connection to Redis Sentinel has failed " + self.conn_retry_count + " times!";
         };
     };

     self.log(error_msg);
     self.notify("Redis Twemproxy Agent Error", error_msg);
  });
  ...
  this.client.on("pmessage", function (p, ch, msg) {
    ...
    self.notify("Redis Twemproxy Agent: Master Switch Occured", JSON.stringify(ret)); 
  });
